...
class CMesh:
	...
	def __init__ (self):
		...
		self.m_pVertices = None 
                # Numeric.array ( (), 'f') 
		# // Vertex Data array
		self.m_pVertices_as_string = None
                # raw memory string for VertexPointer ()

		self.m_pTexCoords = None 
                # Numeric.array ( (), 'f')
                # // Texture Coordinates array
		self.m_pTexCoords_as_string = None
                # raw memory string for TexPointer ()
	...
	def LoadHeightmap( self, szPath, flHeightScale, flResolution ):
		...
		# // Load Texture Data
		try:
			self.m_pTextureImage = Image.open (szPath)
                # // Open The Image
		except:
			traceback.print_exc()
			return False
		...
		# self.m_pVertices = Numeric.zeros ((self.m_nVertexCount * 3), 'f')
                # // Vertex Data
		# Non strings approach
		self.m_pVertices = Numeric.zeros ((self.m_nVertexCount, 3), 'f') 
                # // Vertex Data
		self.m_pTexCoords = Numeric.zeros ((self.m_nVertexCount, 2), 'f') 
                # // Texture Coordinates
		...
		while (nZ < sizeY):
			...
			while (nX < sizeY):
				for nTri in xrange (6):
					# // Using This Quick Hack, Figure The
					# X,Z Position Of The Point
					flX = float (nX)
		...
		self.m_nTextureID = glGenTextures (1)						
                # // Get An Open ID
	...
	def PtHeight (self, nX, nY):
		""" // Calculate The Position In The Texture, Careful Not To
		Overflow """
		sizeX = self.m_pTextureImage.size [0]
		sizeY = self.m_pTextureImage.size [1]
		...
		# Get The Red, Green, and Blue Components 
                # NOTE, Python Image
		# library starts 0 at the top of the image - so to match the
		# windows code we reverse the Y order
