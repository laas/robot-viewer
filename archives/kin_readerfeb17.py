...
def readvrml(filename):
    ...
    keywords=["DEF \w+ Joint","DEF \w* Segment","JointType","jointId","jointAxis"\
          ,"translation","rotation \d \d \d","url "
          ]
    pattern=""
    for key in keywords:
        if pattern=="":
...
import numpy as np
def main():
    base=readvrml(sys.argv[1])
    angles=0.5*np.ones((40,1))
    base.update_angles(angles)
